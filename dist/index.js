    ...
    var Datastore = class extends EventEmitter {
      ...
      _getRawCandidates(query) {
        ...
        usableQuery = Object.entries(query).filter(
          ([k, v]) => !!(typeof v === "string" || typeof v === "number" || typeof v === "boolean" || isDate(v) || v === null) && indexNames.includes(k)
        ).pop();
        if (usableQuery)
...
// src/package.ts
var import_nedb_promises = __toESM(require_nedb_promises(), 1);
...
var Package = class {
  ...
  static async packNedb(packDir, inputDir, compendiumName) {
    const db = import_nedb_promises.default.create(`${packDir}/${compendiumName}.db`);
    const files = fs.readdirSync(inputDir);
    const seenKeys = /* @__PURE__ */ new Set();
    for (const file of files) {
      const fileContents = fs.readFileSync(path.join(inputDir, file), "utf8");
      const value = file.endsWith(".yml") ? js_yaml_default.load(fileContents) : JSON.parse(fileContents);
      ...
      if (existing) {
        ...
        console.log(`Updated ${source_default.blue(value._id)}${source_default.blue(value.name ? ` (${value.name})` : "")}`);
      } else {
        await db.insert(value);
        console.log(`Packed ${source_default.blue(value._id)}${source_default.blue(value.name ? ` (${value.name})` : "")}`);
      }
    }
    const docs = await db.find({ _id: { $nin: Array.from(seenKeys) } });
    for (const doc of docs) {
      await db.remove({ _id: doc._id }, {});
      console.log(`Removed ${source_default.blue(doc._id)}${source_default.blue(doc.name ? ` (${doc.name})` : "")}`);
    }
    db.stopAutocompaction();
    await new Promise((resolve2) => {
      db.compactDatafile(resolve2);
    });
  }
  static async packClassicLevel(packDir, inputDir, ClassicLevel) {
...
// src/utils.ts
async function ensureClassicLevel(tried = false) {
  const installedPath = await (0, import_exec.getExecOutput)("npm", ["ls", "-g", "--parseable", "classic-level"], {
    silent: true
  }).then((out) => {
    if (out.exitCode !== 0)
      return "";
    if (out.stdout.trim() === "")
      return "";
    (0, import_core.info)(`Found classic-level: ${out.stdout}`);
    return out.stdout.trim();
  }).catch(() => "");
  if (installedPath)
    return installedPath;
  if (tried)
    throw new Error("Failed to install classic-level");
  (0, import_core.info)("Installing classic-level");
  await (0, import_exec.exec)("npm", ["install", "-g", "classic-level@1.3.0"]).catch((err) => {
  ...
  return readdir(inputdir).then(async (dir) => {
    for (const subdir of dir) {
      if (statSync(`${inputdir}/${subdir}`).isDirectory()) {
        if (packClassicLevel)
          await Package.packClassicLevel(`${packsdir}/${subdir}`, `${inputdir}/${subdir}`, ClassicLevel).then(() => {
