...
// src/utils.ts
async function ensureClassicLevel() {
  ...
  return readdir(inputdir).then(async (dir) => {
    for (const subdir of dir) {
      if (statSync(`${inputdir}/${subdir}`).isDirectory()) {
        if (packClassicLevel)
          await Package.packClassicLevel(`${packsdir}/${subdir}`, `${inputdir}/${subdir}`).then(() => {
...
// src/index.ts
try {
  const inputDirInput = (0, import_core2.getInput)("inputdir") || "packs";
  const inputdir = resolve(process.cwd(), inputDirInput);
  const packsInput = (0, import_core2.getInput)("packsdir");
  if (!packsInput)
