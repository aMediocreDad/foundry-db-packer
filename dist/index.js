...
Support boolean input list: \`true | True | TRUE | false | False | FALSE\``);
  ...
  async function compilePack(src, dest, {
    ...
    if (nedb && import_path.default.extname(dest) !== ".db") {
  ...
  async function compileNedb(pack, files, { log, transformEntry } = {}) {
    try {
      import_fs.default.unlinkSync(pack);
    } catch (err) {
      if (err.code !== "ENOENT")
        throw err;
    }
    ...
    for (const file of files) {
      try {
        ...
        const doc = import_path.default.extname(file) === ".yml" ? js_yaml_default.load(contents) : JSON.parse(contents);
        const key = doc._key;
  ...
  async function compileClassicLevel(pack, files, { log, transformEntry } = {}) {
    import_fs.default.mkdirSync(pack, { recursive: true });
  ...
  async function extractPack(src, dest, {
    ...
    if (nedb && import_path.default.extname(src) !== ".db") {
  ...
  async function extractNedb(pack, dest, { yaml: asYaml, log, collection, transformEntry, transformName } = {}) {
    ...
    for (const doc of docs) {
      ...
      import_fs.default.writeFileSync(filename, asYaml ? js_yaml_default.dump(doc) : JSON.stringify(doc, null, 2) + "\n");
      if (log)
  ...
  function findSourceFiles(root, { yaml = false, recursive = false } = {}) {
    const files = [];
    for (const entry of import_fs.default.readdirSync(root, { withFileTypes: true })) {
  ...
  var import_fs, import_path, import_nedb_promises, import_classic_level, HIERARCHY, TYPE_COLLECTION_MAP;
  var init_package = __esm({
    "node_modules/@foundryvtt/foundryvtt-cli/lib/package.mjs"() {
      import_fs = __toESM(__require("fs"), 1);
  ...
  async function createDB({
    ...
    return (0, import_promises.readdir)(inputdir).then(async (dir) => {
      for (const subdir of dir) {
        if ((0, import_node_fs.statSync)(`${inputdir}/${subdir}`).isDirectory()) {
          if (packClassicLevel)
  ...
  async function main() {
    try {
      ...
      const inputdir = (0, import_node_path.resolve)(process.cwd(), inputDirInput);
      if (!inputDirInput)
        throw new Error("No packs directory specified");
      const packsInput = (0, import_core2.getInput)("packsdir") || "packs";
      const packsdir = (0, import_node_path.resolve)(process.cwd(), packsInput);
      if (!(0, import_node_fs2.existsSync)(inputdir))
        throw new Error(`Input directory ${inputdir} does not exist`);
      if (!(0, import_node_fs2.existsSync)(packsdir))
