...
var FVTT_CLI_VERSION = "3.0.3";
async function ensureFVTTCli(tried = false) {
  const installedPath = await (0, import_exec.getExecOutput)("npm", ["ls", "-g", "--parseable", "@foundryvtt/foundryvtt-cli"], {
    silent: true
  }).then((out) => {
    if (out.exitCode !== 0) return "";
    if (out.stdout.trim() === "") return "";
    (0, import_core.info)(`Found foundryvtt-cli package: ${out.stdout}`);
    return out.stdout.trim();
  }).catch(() => "");
  if (installedPath) return installedPath;
  if (tried) throw new Error("Failed to install @foundryvtt/foundryvtt-cli");
  (0, import_core.info)(`Installing @foundryvtt/foundryvtt-cli@${FVTT_CLI_VERSION}`);
  await (0, import_exec.exec)("npm", ["install", "-g", `@foundryvtt/foundryvtt-cli@${FVTT_CLI_VERSION}`]).catch((err) => {
    (0, import_core.error)("Error installing @foundryvtt/foundryvtt-cli");
    throw err;
  });
  return ensureFVTTCli(true);
}
async function remove(inputdir) {
...
try {
  ...
  const { compilePack } = await import(`${fvttCliPath}/index.mjs`).catch((err) => {
    ...
    throw new Error(
      `Failed to load foundryvtt-cli from ${fvttCliPath}. This usually means classic-level's prebuilt binary does not match this runner (node ${process.version}, ${process.platform}-${process.arch}). Original error: ${detail}`
    );
  });
  ...
  if ((0, import_core2.getBooleanInput)("remove_input")) await remove(inputdir);
} catch (error2) {
  if (error2 instanceof Error) (0, import_core2.setFailed)(error2.message);
  else (0, import_core2.setFailed)("Unknown error");
  process.exit(1);
}
